...
class WBoard {
  ...
  protected wtsy_lib = class WLib {
    ...
    public Terminal = class WTerminal {
      ...
      public adicionaColorizacaoTexto(modelo: WCollections, tC: "3b4bit"|"8bit"|"24bit", eC: "C"|"F", texto: string): string {
        let formato = null;
        switch(tC) {
          case "3b4bit":
            switch(eC) {
              case "C":
                formato = /[0-9]m/gi.exec(modelo.Terminal.Tipo.Coloracao.Template.Formato.f_3b4bit.CorLetra)?.toString();
              case "F":
                formato = /[0-9]m/gi.exec(modelo.Terminal.Tipo.Coloracao.Template.Formato.f_3b4bit.CorFundo)?.toString();
            }
          case "8bit":
            switch(eC) {
              case "C":
                formato = /[0-9];5;[0-9]m/gi.exec(modelo.Terminal.Tipo.Coloracao.Template.Formato.f_8bit.CorLetra)?.toString();
              case "F":
                formato = /[0-9];5;[0-9]m/gi.exec(modelo.Terminal.Tipo.Coloracao.Template.Formato.f_8bit.CorFundo)?.toString();
            }
          case "24bit":
            switch(eC) {
              case "C":
                formato = /[0-9];2;[0-9];[0-9];[0-9]m/gi.exec(modelo.Terminal.Tipo.Coloracao.Template.Formato.f_24bit.CorLetra)?.toString();
              case "F":
                formato = /[0-9];2;[0-9];[0-9];[0-9]m/gi.exec(modelo.Terminal.Tipo.Coloracao.Template.Formato.f_24bit.CorFundo)?.toString();
            }
        }
        return `\033[${formato}${texto}\033[0m`;
      }
