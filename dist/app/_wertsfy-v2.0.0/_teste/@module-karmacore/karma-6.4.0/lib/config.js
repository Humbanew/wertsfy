...
function parseConfig(configFilePath, cliOptions, parseOptions) {
    ...
    function fail() {
        ...
        if (throwErrors) {
            ...
            const warningMessage = 'The `parseConfig()` function historically called `process.exit(1)`' +
     ...
     */
    const returnIsThenable = (((configModuleReturn != null && typeof configModuleReturn === 'object') ||
        typeof configModuleReturn === 'function') && typeof configModuleReturn.then === 'function');
    if (returnIsThenable) {
        if (promiseConfig !== true) {
            const errorMessage = 'The `parseOptions.promiseConfig` option must be set to `true` to ' +
