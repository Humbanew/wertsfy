...
describe('middleware.karma', () => {
    ...
    beforeEach(() => {
        ...
        handler = createKarmaMiddleware(filesDeferred.promise, serveFile, null, injector, '/base/path', '/__karma__/', { path: '/__proxy__/' });
    });
    // helpers
    ...
    it('should serve client.html', (done) => {
        handler = createKarmaMiddleware(null, serveFile, null, injector, '/base', '/');
        response.once('end', () => {
            expect(nextSpy).not.to.have.been.called;
    ...
    it('should serve context.html with included DOM content', (done) => {
        filesDeferred = helper.defer();
        handler = createKarmaMiddleware(filesDeferred.promise, serveFile, null, injector, '/base', '/');
        includedFiles([
    ...
    it('should update handle updated configs', (done) => {
        ...
        }, '/base/path', '/__karma__/', { path: '/__proxy__/' });
        includedFiles([
            new MockFile('/first.js')
        ]);
        fsMock._touchFile('/karma/static/debug.html', 1, '%CLIENT_CONFIG%');
...
//# sourceMappingURL=karma.spec.js.map