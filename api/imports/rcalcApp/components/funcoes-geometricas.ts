export function $areaTrianguloQualquer(comp: number, larg: number) { 
  return comp == larg ? (comp * comp * Math.sqrt(3)) / 4 : comp * larg / 2; 
}

export function $apotemaDeUmTrianguloQualquer(comp: number, larg: number) {
  return comp == larg ? (comp * comp * Math.sqrt(3)) / 12 : comp * larg / (4 * Math.sqrt(3));
}
