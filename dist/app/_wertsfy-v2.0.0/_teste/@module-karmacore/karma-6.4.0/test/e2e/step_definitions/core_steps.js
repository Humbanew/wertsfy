...
When('I wait until server output contains:', async function (expectedOutput) {
    await waitForCondition(() => this.backgroundProcess.stdout.includes(expectedOutput), 5000, () => new Error('Expected server output to contain the above text within 5000ms, but got:\n\n' +
...
Then(/^The (server|stopper) is dead(:? with exit code (\d+))?$/, function (stopperOrServer, code, callback) {
    ...
    setTimeout(() => {
        ...
        if (actualExitCode === undefined)
            return callback(new Error('Server has not exited.'));
        if (code === undefined || parseInt(code, 10) === actualExitCode)
            return callback();
...
Then(/^the background (stdout|stderr) (is exactly|contains|matches RegExp):$/, async function (outputType, comparison, expectedOutput) {
    ...
    }, 5000, () => new Error(`${message}\n\n${this.backgroundProcess[outputType]}`));
});
//# sourceMappingURL=core_steps.js.map