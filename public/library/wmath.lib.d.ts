  /** @description Constante de Arquimedes ou número de Ludoph. */
  export declare const PI = 3.141592653589793;

  /** @description Constante de Euler. */
  export declare const E = 2.7182818284590452353602874713527;

  /** @description Constante de Planck.*/
  export declare const PLANCK = 6.62606896e-34;

  /** @description Constante de Avogadro.*/
  export declare const NA = 6.022140857e23;

  /** @description Constante de Newton.*/
  export declare const NEWTON = 0.6931471805599453;

  /** @description Constante de Tesla.*/
  export declare const TESLA = 1.6021766208e-19;

  /** @description Constante da Raiz de um. */
  export declare const SQRT1 = 1.0;

  /** @description Constante da Raiz de dois. */
  export declare const SQRT2 = 1.414213562373095;

  /** @description Constante da Raiz de tres. */
  export declare const SQRT3 = 1.732050807568877;

  /** @description Constante da Raiz de quatro. */
  export declare const SQRT4 = 2.0;

  /** @description Constante da Raiz de cinco. */
  export declare const SQRT5 = 2.23606797749979;

  /** @description Constante da Raiz de seis. */
  export declare const SQRT6 = 2.449489742783178;

  /** @description Constante da Raiz de sete. */
  export declare const SQRT7 = 2.64575131106459;

  /** @description Constante da Raiz de oito. */
  export declare const SQRT8 = 2.82842712474619;

  /** @description Constante da Raiz de nove. */
  export declare const SQRT9 = 3.0;

  /** @description Constante da Raiz de dez. */
  export declare const SQRT10 = 3.16227766016838;

  /** @description Constante da Raiz Cúbica de um. */
  export declare const CBRT1 = 1.0;

  /** @description Constante da Raiz Cúbica de dois. */
  export declare const CBRT2 = 1.25992104989487;

  /** @description Constante da Raiz Cúbica de tres. */
  export declare const CBRT3 = 1.5874010519682;

  /** @description Constante da Raiz Cúbica de quatro. */
  export declare const CBRT4 = 1.99999999999999;

  /** @description Constante da Raiz Cúbica de cinco. */
  export declare const CBRT5 = 2.23606797749979;

  /** @description Constante da Raiz Cúbica de seis. */
  export declare const CBRT6 = 2.58198889747161;

  /** @description Constante da Raiz Cúbica de sete. */
  export declare const CBRT7 = 2.91293118277239;

  /** @description Constante da Raiz Cúbica de oito. */
  export declare const CBRT8 = 3.26249509498958;

  /** @description Constante da Raiz Cúbica de nove. */
  export declare const CBRT9 = 3.62449963442055;

  /** @description Constante da Raiz Cúbica de dez. */
  export declare const CBRT10 = 3.99999999999999;

  /** @description Constante de Logaritmo Natural de dois. */
  export declare const LN2 = 0.6931471805599453;

  /** @description Constante de Logaritmo Natural de dez. */
  export declare const LN10 = 2.302585092994046;

  /** @description Constante de Logaritmo de 2. */
  export declare const LOG2E = 1.4426950408889634;

  /** @description Constante de Logaritmo de 10. */
  export declare const LOG10E = 0.4342944819032518;

  /** @description Maior valor numérico preciso possível.*/
  export declare const MAX_VALUE = 999999999999999;

  /**@description Menor valor númerico preciso possível.*/
  export declare const MIN_VALUE = -999999999999999;
