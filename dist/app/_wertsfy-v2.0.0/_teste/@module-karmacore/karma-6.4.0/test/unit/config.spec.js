...
describe('config', () => {
    ...
    const normalizeConfigWithDefaults = (cfg) => {
        ...
        if (!cfg.exclude)
            cfg.exclude = [];
        if (!cfg.junitReporter)
            cfg.junitReporter = {};
        if (!cfg.coverageReporter)
    ...
    describe('parseConfig', () => {
        ...
        it('should log an error and exit if file does not exist', () => {
            ...
            expect(event.toString().split('\n').slice(0, 2)).to.be.deep.equal(['Error in config file!', '  Error: Cannot find module \'/conf/not-exist.js\'']);
        ...
        it('should log error and throw if file does not export a function AND throwErrors is true', () => {
            ...
            expect(logErrorStub).to.have.been.called;
            const event = logErrorStub.lastCall.args;
            expect(event.toString().split('\n').slice(0, 1)).to.be.deep.equal(['Config file must export a function!']);
            expect(mocks.process.exit).not.to.have.been.called;
        });
        it('should log an error and fail when the config file\'s function returns a promise, but `parseOptions.promiseConfig` is not true', () => {
            function parseConfig() {
                e.parseConfig('/conf/returns-promise-that-resolves.js', {}, { throwErrors: true });
            }
            const expectedErrorMessage = 'The `parseOptions.promiseConfig` option must be set to `true` to ' +
        ...
        describe('when `parseOptions.promiseConfig` is true', () => {
            it('should return a promise when promiseConfig is true', () => {
                ...
                const noConfigFilePromise = e.parseConfig(null, null, { promiseConfig: true });
                const syncConfigPromise = e.parseConfig('/conf/default-config', null, { promiseConfig: true });
                const asyncConfigPromise = e.parseConfig('/conf/returns-promise-that-resolves.js', null, { promiseConfig: true });
                expect(noConfigFilePromise).to.be.an.instanceof(Promise, 'Expected parseConfig to return a promise when no config file path is provided.');
                expect(syncConfigPromise).to.be.an.instanceof(Promise, 'Expected parseConfig to return a promise when the config file DOES NOT return a promise.');
                expect(asyncConfigPromise).to.be.an.instanceof(Promise, 'Expected parseConfig to return a promise when the config file returns a promise.');
            });
            it('should log an error and exit if invalid file', () => {
